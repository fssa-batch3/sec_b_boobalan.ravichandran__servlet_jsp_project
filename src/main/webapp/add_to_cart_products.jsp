...
<body>
	...
	<main>
		...
		<div class="cart" id="cartRemove">
			...
			<div class="cart11"></div>
		</div>
		<img id="empty-cart-image"
...
async function getAllProducts() {
    try {
        ...
        processAllProductsDetails(loadProduct, findLogedUserAddToCart);
    } catch (error) {
...
function processAllProductsDetails(loadProducts, cartQuantity) {
    ...
    loadProducts.forEach((loadProduct) => {
        ...
        let quantityValue = 0; // Use 'let' instead of 'const'
        for (const cartItem of cartQuantity) {
        	
            if (loadProduct.id == cartItem.cart_id) {
            	
            	quantityValue = parseInt(cartItem.product_qty, 10);
     ...
     // Create and append option elements for quantity
        ...
        let selectedQuantity = quantityValue; // Default to 1
...
function updateCartDetails(loadProduct, selectedQuantity) {
 ...
 // discountElement.textContent = 'You Save: \u20B9 ' + newDiscount;
  ...
  const productId = loadProduct.id;
  let addToCartItemsDetails = JSON.parse(localStorage.getItem('addToCartItem')) || [];

  // Check if there is already an object with the same product ID
  const existingItem = addToCartItemsDetails.find((item) => item.cart_id == productId && item.userUniqueId == logedUser);
  
  if (existingItem) {
    // Update the quantity for the existing item
    existingItem.product_qty = selectedQuantity+"";
  } 

  // Store the updated addToCartItem in local storage
  localStorage.setItem('addToCartItem', JSON.stringify(addToCartItemsDetails));
...
selectQuantity.addEventListener('change', function () {
  ...
  const selectedQuantity = parseInt(selectQuantity.value);

  // Call the updateCartDetails function to update details and store quantity
  updateCartDetails(loadProduct, selectedQuantity);
});
        ...
        const discount = 1 * loadProduct.productDiscount;
        const actualPrice = 1 * loadProduct.productPrice;
        ...
        cartQuantity.forEach((cartQty) => {
        	if(cartQty.userUniqueId == logedUser && cartQty.cart_id == loadProduct.id )
        		proQty = parseInt(cartQty.product_qty);
        	// console.log("loadProduct.productPrice*proQty"+ proQty);
        	
        });
       
        // Accumulate price1 and discount
        const price1 = (loadProduct.productPrice*proQty);
        const disc = (loadProduct.productDiscount*proQty);
        totalPrice += price1;
...
// remove product from localstorage
    function removeProductFromCart(cartId) {
    	  ...
    	  if (confirmed) {
    	    ...
    	    const index = cart.findIndex((item) => item.cart_id == cartId && item.userUniqueId == logedUser);
